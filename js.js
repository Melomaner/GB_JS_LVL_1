...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.usedCells.push(foodCell); // foodPoint -> foodCell
  },
};
...
const snake = {
  ...
  /**
   * Для следующего шага устанавливается телепорт.
   */
  setTeleportStep(){
    this.teleportStep = true;
  },
  ...
  getNextStepHeadPoint() {
    ...
    if (!this.teleportStep){
      ...
      switch (this.direction) {
        case 'up':
          return {x: firstPoint.x, y: 20};
        case 'right':
          return {x: 0, y: firstPoint.y};
        case 'down':
          return {x: firstPoint.x, y: 0};
        case 'left':
          return {x: 20, y: firstPoint.y};
      }
    }
...
// Сбрасываем счет
  init(){
    this.score = 0
    document.getElementById("score").innerHTML = `Счет игры: ${this.score}`
  },
  // Добовляем +1 поинт
  addScore(){
    this.score += 1
...
const game = {
  ...
  tickHandler() {
    ...
    // Если следующий шаг вне поля, то перемещаемся на другую сторону.
    if (!this.teleportStep()){
      console.log(this.teleportStep())
      this.snake.setTeleportStep()
    }
    ...
    if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      ...
      this.food.setCoordinates(this.getRandomFreeCoordinates());
      // Добавляем пойнт к нашему счету.
      this.score.addScore()
  ...
  /**
   * Проверяет будет ли следующий шаг вне поля.
   * @returns {boolean} true если шаг обычный, false если следующий шаг вне поля.
   */
  teleportStep(){
    const nextHeadPoint = this.snake.getNextStepHeadPoint();
...
window.onload = game.init({speed: 10});